...
`--test-only` sets `TEST_ONLY=1`: it requires an existing schema, skips schema creation and checkpoint loading, redirects checkpoint output to `/dev/null`, and rolls every writer transaction back after `COPY`. It changes 0 persistent files and 0 PostgreSQL rows, but cannot measure durable commit cost. `--dry-run` executes 0 workloads and changes 0 files. `--apply-to ENV_FILE` offers a separate confirmation prompt for every recommendation; rejecting one setting leaves it unchanged and continues with the remaining settings. The `{docker_env}` command token expands to Docker `-e NAME=VALUE` arguments.
